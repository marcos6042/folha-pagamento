os dados abaixo tem o código completo? ajuste para ter o código completo
...
export default function Listagem() {
  ...
  const fetchData = async () => {
    const fetchAll = async (table, setter) => {
      ...
      if (table === "eventos_folha" && competencia) {
    ...
    fetchAll("empresas", setEmpresas);
    fetchAll("trabalhadores", setTrabalhadores);
    fetchAll("rubricas", setRubricas);
    fetchAll("vinculos", setVinculos);
    fetchAll("eventos_folha", setEventos);
    fetchAll("estabelecimentos", setEstabelecimentos);
  };

  const handleDelete = async (table, idField, id, refetch) => {
    ...
    datasets: [
      {
        ...
        data: Object.values(datasets).map((d) => d.length),
  ...
  return (
    <div className="p-4 space-y-10">
      ...
      <div className="flex flex-wrap gap-4 mb-4">
        <input
          type="text"
          placeholder="Buscar por nome, razão social ou descrição"
          value={filtro}
        ...
        {datasets[selectedTable].length > 0 && (
          <CSVLink
...
function Section({ title, data, table, idField, refetch }) {
            ...
            {data.map((row, idx) => (
              <tr key={idx} className="border-t">
                ...
                <td className="px-2 py-1 border-b">
                  <button onClick={() => handleDelete(table, idField, row[idField], refetch)} className="text-red-600 hover:underline">Excluir</button>
                </td>
              </tr>
            ))}
          </tbody>
...
// Código completo com edição inline, painel XML e gráfico já implementado.
// Por motivos de espaço, dividiremos esses recursos em partes se necessário.
// A seguir vem a primeira atualização com edição inline usando um modal e exportação XML simulada.
...
// 🧩 Aguardando confirmação para inserir edição inline e exportação XML de forma estruturada em múltiplos componentes se necessário.
