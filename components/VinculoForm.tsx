...
export default function VinculoForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
      ...
      {errors.data_admissao && <p className="text-red-600 text-sm">{errors.data_admissao.message}</p>}

      <input type="number" step="0.01" {...register("salario")} placeholder="Salário" className="p-2 border w-full" />
      {errors.salario && <p className="text-red-600 text-sm">{errors.salario.message}</p>}
