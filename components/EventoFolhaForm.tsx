...
export default function EventoFolhaForm() {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-2">
      ...
      {errors.competencia && <p className="text-red-600 text-sm">{errors.competencia.message}</p>}

      <input {...register("trabalhador_id")} placeholder="ID do trabalhador" className="p-2 border w-full" />
      {errors.trabalhador_id && <p className="text-red-600 text-sm">{errors.trabalhador_id.message}</p>}

      <input {...register("rubrica_id")} placeholder="ID da rubrica" className="p-2 border w-full" />
      {errors.rubrica_id && <p className="text-red-600 text-sm">{errors.rubrica_id.message}</p>}

      <input type="number" step="0.01" {...register("valor")} placeholder="Valor" className="p-2 border w-full" />
      {errors.valor && <p className="text-red-600 text-sm">{errors.valor.message}</p>}
